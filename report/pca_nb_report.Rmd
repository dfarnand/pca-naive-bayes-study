...
```{r setup, echo=FALSE, message=FALSE, warning=FALSE, output=FALSE, cache=TRUE}
library(tidyverse)
library(ggthemr)
library(ggrepel)
library(here)
...
results_tab %>%
  ...
  geom_text_repel() +
  labs(title = "Entropy vs PCA Improvement",
       y = "PCA Diff", 
       color = "")
...
results_tab %>%
  ...
  geom_smooth(aes(color = "Regression Line"), method = 'lm', se = FALSE)+
  geom_text_repel() +
  labs(title = "MSA vs PCA Improvement",
